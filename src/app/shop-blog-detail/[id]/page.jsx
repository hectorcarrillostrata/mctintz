...
import { shopBlog } from '@/app/data/shop'

export default function ShopBlogDetail({params}) {
    let {id} = use(params)
    let data = shopBlog.find((item)=>item.id === parseInt(id))
  return (
    <>
     <ShopNavbar/> 
          ...
          <div className="row justify-content-center mt-5">
              <div className="col-lg-8">
                  <div className="section-title">
                      <span className="badge text-bg-primary" style={{margin:'0 2px'}}>{data?.tag}</span>
                      <span className="badge text-bg-primary" style={{margin:'0 2px'}}>Men</span>
                      <h4 className="title fw-medium mb-4 mt-2">{data?.title}</h4>
