...
export default function JobDetailOne({params}) {
    let {id} = use(params)

    let data = featureJob.find((item)=>item.id === parseInt(id));
          ...
          <div className="row mt-5 justify-content-center">
              <div className="col-lg-12 text-center">
                  <div className="pages-heading">
                      <Image src={data?.image} width={80} height={80} className="avatar avatar-md-md rounded-pill bg-white p-3" alt=""/>
                      <h4 className="title text-white title-dark mt-4 mb-3">{data?.name}</h4>
                      ...
                      <ul className="list-unstyled mb-0">
                          <li className="list-inline-item text-white-50 me-3"><i className="mdi mdi-map-marker text-warning me-2"></i>{data?.location}</li>
                          <li className="list-inline-item text-white-50"><i className="mdi mdi-office-building text-warning me-2"></i>{data?.company}</li>
                      </ul>
                  </div>
              </div> 
          </div>
