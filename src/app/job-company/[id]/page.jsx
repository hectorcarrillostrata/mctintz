...
import { companyList, jobdata } from '../../data/job'


export default function JobCompany({params}) {
    let {id} = use(params)
    let data = companyList.find((item)=>item.id === parseInt(id))
  return (
    <>
                  ...
                  <div className="job-profile position-relative">
                      <div className="card rounded shadow">
                          <div className="text-center py-5 border-bottom">
                              <Image src={data?.image} width={110} height={110} className="avatar avatar-medium mx-auto rounded-circle d-block" alt=""/>
                          ...
                          <div className="p-4">
                              ...
                              <ul className="list-unstyled my-4">
                                  <li className="h6 mb-2"><FiMapPin className="fea icon-sm text-warning me-3"/><span className="text-muted">Location :</span>{data?.location}</li>
                                  <li className="h6 mb-2"><LuLink className="fea icon-sm text-warning me-3"/><span className="text-muted">Comapny :</span> {data?.name}.com</li>
                                  <li className="h6 mb-2"><FiDollarSign className="fea icon-sm text-warning me-3"/><span className="text-muted">Revenue :</span>{data?.revenue}</li>
                                  <li className="h6 mb-2"><FiUsers className="fea icon-sm text-warning me-3"/><span className="text-muted">No. of employees :</span>{data?.employees}</li>
                              </ul>
